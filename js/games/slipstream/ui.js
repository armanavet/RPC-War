/* ============================================================
   Slipstream: everything that touches the DOM.

   Satisfies the UI CONTRACT in js/net/matchsync.js — $, render,
   renderMpPanels, renderChat, setFlip, setLink, flashCopied, mpErr.

   Two things this board has to show that a normal board does not:
   the trail a slide will travel, and how close the next ring is to
   falling. Both are the whole game, so both are drawn loudly.
   ============================================================ */
import {SZ, BLUE, RED, col, isWall, rowOf, colOf, ringOf, ringsClosed,
        isClosed, untilShrink, pathOf} from './rules.js';
...
export function initBoard(handlers){
  ...
  boardEl.addEventListener('pointerdown', e => {
    ...
    if(!p || isWall(p) || col(p) !== game.turn) return;
...
function drawCells(){
  ...
  const doomed = ringsClosed(ply);          // the ring that falls next
  const soon = untilShrink(ply);
  for(let i = 0; i < SZ; i++){
    const d = cellDivs[dsp(i)];
    let cls = 'cell ' + (((rowOf(i) + colOf(i)) % 2) ? 'dark' : 'lite');
    if(isClosed(i, ply)) cls += ' gone';
    else if(isWall(game.bd[i])) cls += ' wall';
    else if(soon !== null && soon <= 4 && ringOf(i) === doomed) cls += ' doomed';
...
/* The trail is the point: you are choosing a direction, not a distance. */
function drawMarks(){
  marksEl.innerHTML = '';
  if(game.sel < 0) return;
  for(const m of game.legal){
    for(const step of pathOf(game.sel, m.to)){
      const t = document.createElement('i');
      t.className = 'trail';
      t.style.left = (colOf(dsp(step)) * 100 / 9) + '%';
      t.style.top  = (rowOf(dsp(step)) * 100 / 9) + '%';
      marksEl.appendChild(t);
    }
    cellDivs[dsp(m.to)].classList.add('pick');
    const s = document.createElement('div');
    s.className = 'mv' + (m.o === 'capture' ? ' mv--take' : '');
    s.style.left = (colOf(dsp(m.to)) * 100 / 9) + '%';
    s.style.top  = (rowOf(dsp(m.to)) * 100 / 9) + '%';
    s.innerHTML = m.o === 'capture' ? '<i class="ring"></i>' : '<i class="dot0"></i>';
...
function drawStatus(){
  ...
  if(net.on){
    ...
    hint.textContent = game.over ? '' : game.busy ? 'Thinking…'
      : (game.sel >= 0 ? 'Pick a direction' : 'Pick a piece');
  }
  $('ply').textContent = Math.ceil(game.ply / 2);

  const soon = untilShrink(game.moveList.length);
  const el = $('shrink');
  if(!el) return;
  if(soon === null){ el.textContent = 'board fully closed'; el.className = 'shrink is-done'; }
  else{
    el.textContent = soon === 1 ? 'ring falls next move' : 'ring falls in ' + soon;
    el.className = 'shrink' + (soon <= 4 ? ' is-soon' : '');
  }
}
