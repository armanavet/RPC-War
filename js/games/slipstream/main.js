/* ============================================================
   Slipstream: wiring. The only module that knows about all the
...
const SLUG = 'slipstream';

/* ---------- computer player ---------- */
const aiEnabled = () => ui.$('aiOn').checked;

const LEVELS = {
  1: (bd, ply) => bestMove(bd, RED, 2, true, ply),
  2: (bd, ply) => bestMoveTimed(bd, RED, 4, 600, false, ply),
  3: (bd, ply) => bestMoveTimed(bd, RED, 8, 1800, false, ply),
};
...
function maybeAI(){
  ...
  setTimeout(() => {
    ...
    const mv = (LEVELS[lv] || LEVELS[2])(game.bd, game.moveList.length);
    game.busy = false;
    if(mv == null){
      game.over = {w: BLUE, why: 'nowhere left to slide'};
      ui.render();
      return;
    }
    move(mv);
